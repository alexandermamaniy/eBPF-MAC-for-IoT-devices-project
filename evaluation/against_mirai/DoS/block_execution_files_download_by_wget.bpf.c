...
int BPF_PROG(mark_wget_inodes, struct file *file, int flags)
{
    ...
    */
    ...
    bpf_printk("Marked inode %llu as created by %s\n", ino, comm);
    return 0;
}
...
int BPF_PROG(block_marked_exec, struct linux_binprm *bprm)
{
    ...
    if (found) {
        bpf_printk("Blocking execution of inode %llu (wget)\n", ino);
        return -EPERM;
    }
    return 0;
}
...
int BPF_PROG(block_marked_chmod, struct dentry *dentry, struct iattr *attr)
{
    ...
    if (!(attr->ia_valid & (1 << 0))) // ATTR_MODE
        return 0;
    ...
    u8 *found = bpf_map_lookup_elem(&marked_inodes, &ino);
    if (found) {
        bpf_printk("Blocking chmod on inode %llu (wget)\n", ino);
        return -EPERM;
    }
    return 0;
}
